...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a persona-aware backlog summary (Markdown).")
    parser.add_argument(
        "--source",
...
def parse_frontmatter_quick(path: Path) -> Dict[str, str]:
...
def iter_items_from_files(items_root: Path, product_name: str) -> Iterable[ItemRow]:
    for path in items_root.rglob("*.md"):
        ...
        if path.name == "README.md" or path.name.endswith(".index.md"):
            continue
        data = parse_frontmatter_quick(path)
...
def iter_items_from_sqlite(db_path: Path, products: Sequence[str]) -> Iterable[ItemRow]:
    # Index DBs are typically product-scoped, so we read all rows without filtering by product.
    # (The schema still contains a `product` column for future cross-product scenarios.)
    conn = sqlite3.connect(str(db_path))
    try:
        rows = conn.execute(
            "SELECT COALESCE(product, ''), id, type, title, COALESCE(state, ''), COALESCE(priority, 'P2'), COALESCE(updated, '') "
            "FROM items"
        ).fetchall()
        for r in rows:
            product_name, item_id, item_type, title, state, priority, updated = r
            if not item_id or not item_type or not state:
                continue
            yield ItemRow(
                product=str(product_name) if product_name else "",
                id=str(item_id),
                type=str(item_type),
...
def select_items(rows: Sequence[ItemRow], *, states: Sequence[str], types: Optional[Sequence[str]] = None) -> List[ItemRow]:
    ...
    for r in rows:
        ...
        if wanted_types is not None and r.type not in wanted_types:
            continue
        selected.append(r)
    selected.sort(key=lambda x: (_parse_priority(x.priority), x.updated or "", x.id))
    return selected


def render_list(title: str, rows: Sequence[ItemRow], limit: int) -> List[str]:
    lines = [f"## {title}", ""]
    if not rows:
        lines.append("- (none)")
        lines.append("")
...
def main() -> int:
    ...
    def collect_for_root(backlog_root: Path, display_product: str) -> Tuple[List[ItemRow], str, Dict[str, object]]:
        ...
        if use_sqlite:
            rows = list(iter_items_from_sqlite(db_path, []))
            source_label = f"sqlite:{db_path.relative_to(repo_root) if repo_root in db_path.parents else db_path}"
            # Fill missing product labels from context.
            if display_product:
                rows = [ItemRow(product=(r.product or display_product), **{k: getattr(r, k) for k in ("id","type","title","state","priority","updated")}) for r in rows]
            return rows, source_label, config

        items_root = backlog_root / "items"
        rows = list(iter_items_from_files(items_root, product_name=display_product or ""))
    ...
    lines: List[str] = []
    lines.append(f"# Project Summary ({persona})")
    lines.append("")
    lines.append(f"- Generated: {now}")
    lines.append(f"- Source: `{source_label}`")
    if products:
        lines.append(f"- Products: {', '.join(f'`{p}`' for p in products)}")
    elif product_name:
        lines.append(f"- Product: `{product_name}`")
    lines.append("")

    # Counts by group/state.
    ...
    lines.append("")
    lines.append(f"- New: **{counts_by_group.get('New', 0)}**")
    lines.append(f"- InProgress: **{counts_by_group.get('InProgress', 0)}**")
    lines.append(f"- Done: **{counts_by_group.get('Done', 0)}**")
    lines.append("")

    # Persona-specific highlights (deterministic).
    if persona == "pm":
        lines += render_list(
            "Active Epics / Features",
            select_items(rows, states=["InProgress", "Review", "Blocked", "Ready", "Planned"], types=["Epic", "Feature"]),
            limit=10,
        )
        lines += render_list(
            "New Proposals (Epics / Features)",
            select_items(rows, states=["Proposed"], types=["Epic", "Feature"]),
            limit=10,
        )
    elif persona == "qa":
        lines += render_list(
            "Bugs to Triage",
            select_items(rows, states=["Proposed", "Planned", "Ready", "InProgress", "Blocked", "Review"], types=["Bug"]),
            limit=15,
        )
        lines += render_list(
            "Items in Review (verification focus)",
            select_items(rows, states=["Review"], types=None),
            limit=15,
        )
    else:  # developer
        lines += render_list(
            "InProgress (continue)",
            select_items(rows, states=["InProgress", "Review", "Blocked"], types=["Task", "Bug"]),
            limit=15,
        )
        lines += render_list(
            "Ready (next)",
            select_items(rows, states=["Ready"], types=["Task", "Bug"]),
            limit=15,
        )
