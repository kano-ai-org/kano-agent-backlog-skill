...
- `scripts/backlog/`: backlog helpers (create/update/validate, views, indexing)
- `scripts/fs/`: file ops (copy/move/delete/trash) constrained to `_kano/` roots
- `scripts/logging/`: audit logging (JSONL tool invocation log + redaction)
...
3) Open `_kano/backlog/views/` (or generate views using `scripts/backlog/view_refresh_dashboards.py`)
